...
def build_meta(proxy, qparams, entity):
    """"
    Builds the `meta` part of the response
    We assume that receivedRequest is the evaluated request (qparams) sent by the user.
    """
    
    schemas = [ { "entityType": paramaters_to_entity(qparams), "schema": qparams.requestedSchema[0] } ]

    meta = {
        'beaconId': config.beacon_id,
        'apiVersion': config.api_version,
        'receivedRequest': build_received_request(qparams, schemas, entity),
        'receivedRequestSummary':  build_received_request_summary(qparams, schemas),
...
def build_received_request(qparams, schemas, entity):
...
def build_received_request_summary(qparams, schemas):
...
def build_received_query(qparams, entity):
